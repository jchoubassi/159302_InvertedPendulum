...
fl->inp_mem_fns[in_theta][in_nm] = init_trapz(0.00, 0.00, -0.55,  -0.40, left_trapezoid);
fl->inp_mem_fns[in_theta][in_ns] = init_trapz(-0.26, -0.25,  -0.25,  0.15, regular_trapezoid);
fl->inp_mem_fns[in_theta][in_ze] = init_trapz(-0.10, 0.00,  0.00,  0.10, regular_trapezoid);
fl->inp_mem_fns[in_theta][in_ps] = init_trapz( 0.15,  0.25,  0.25,  0.26, regular_trapezoid);
fl->inp_mem_fns[in_theta][in_pm] = init_trapz( 0.40,  0.55,  0.00,  0.00, right_trapezoid);

// Y Membership Functions 

fl->inp_mem_fns[in_x][in_nm] = init_trapz(0.00, 0.00, -1.30, -0.8, left_trapezoid);
fl->inp_mem_fns[in_x][in_ns] = init_trapz(-1.152, -0.75,  -0.75,  -0.10, regular_trapezoid);
fl->inp_mem_fns[in_x][in_ze] = init_trapz( -0.59,  0.00,  0.00,  0.60, regular_trapezoid);
fl->inp_mem_fns[in_x][in_ps] = init_trapz( 0.10,  0.70,  0.75,  1.14, regular_trapezoid);
fl->inp_mem_fns[in_x][in_pm] = init_trapz( 0.77,  1.30,  0.00,  0.00, right_trapezoid);
...
void initFuzzySystem (fuzzy_system_rec *fl) {
   ...
   fl->output_values[out_pm] = 220.0;
   fl->output_values[out_pl] = 230.0;
